...
module.exports.confirm = function(req, res) {
                                    ...
                                    if (err) {
                                        ...
                                        if (result.length === 0) {
                                            ...
                                            if (result[0].status === 'false') {
                                                ...
                                                let sql2 = 'SELECT * FROM ((bookings  INNER JOIN users ON bookings.user_id = users.id)  INNER JOIN rooms ON bookings.room_id = rooms.room_id) where room_name= '+"'"+req.body.room_name+"'";
                                                con.query(sql2, function (err, result) {
                                                    if (err) {
                                                        res.sendStatus(400);
                                                    } else {
                                                        if (result.length === 0) {
                                                            var sql3 = 'select * from rooms where room_name='+"'"+req.body.room_name+"'";
                                                            con.query(sql3, function (err, result) {
                                                                if (err) {
                                                                    ...
                                                                    for (let i=0;i<result.length; i++){
                                                                        validateResult = validateBooking(req.body.booking_start_time, req.body.booking_end_time, result[i].room_start_time, result[i].room_end_time);
                                                                        if (validateResult !==0){
                                                                            break;
                                                                        }
                                                                    }
    ...
    function validateBooking(reqStartTime, reqEndTime, roomStartTime, roomEndTime) {

        var start=(reqStartTime).substr(16, 8).split(":");
        var end = (reqEndTime).substr(16, 8).split(":");
        var startTimeVal = parseInt(start[0])*3600+parseInt(start[1])*60;
        var endTimeVal = parseInt(end[0])*3600+parseInt(end[1])*60;

        var roomStart=(roomStartTime).substr(16, 8).split(":");
        var roomEnd = (roomEndTime).substr(16, 8).split(":");
        var roomStartTimeVal = parseInt(roomStart[0])*3600+parseInt(roomStart[1])*60;
        var roomEndTimeVal = parseInt(roomEnd[0])*3600+parseInt(roomEnd[1])*60;
    ...
    function validateBookingInfo( reqRoomName, roomName, startTime, endTime, bookingStartTime, bookingEndTime, roomStartTime, roomEndTime) {
        ...
        var bookingStartTimeVal = parseInt(bookingStart[0])*3600+parseInt(bookingStart[1])*60;
        var bookingEndTimeVal = parseInt(bookingEnd[0])*3600+parseInt(bookingEnd[1])*60;
